...
import { A as AstroError, U as UnknownContentCollectionError, c as createComponent, R as RenderUndefinedEntryError, u as unescapeHTML, r as renderTemplate, f as renderUniqueStylesheet, g as renderScriptElement, h as createHeadAndContent, a as renderComponent } from './astro/server_DfsTNRNX.mjs';
import 'piccolore';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_CuKNjM8B.mjs');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_DyRI0aj6.mjs').then(n => n._);
...
function Tag({ tag, className = "" }) {
  ...
  return /* @__PURE__ */ jsx("span", { className: `px-2 py-1 text-xs rounded ${color.bg} ${color.text} ${className}`, children: tag });
}
function TagList({ tags, className = "" }) {
  if (!tags || tags.length === 0) return null;
  return /* @__PURE__ */ jsx("div", { className: `flex flex-wrap gap-2 ${className}`, children: tags.map((tag) => /* @__PURE__ */ jsx(Tag, { tag }, tag)) });
}

export { Tag as T, getCollection as a, TagList as b, getTagColor as g };
